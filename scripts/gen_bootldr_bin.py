...
def process_bootldr(cdt_info):
        ...
        if os.path.exists(bootldr_gen):
                os.chmod(bootldr_gen, 0o744)
        ...
        print('\tCreating CDT binary', end=' ')
        prc = subprocess.Popen([sys.executable, cdt_gen, cdt_info_file, cdtbrdbin],
                        cwd=outputdir)
        prc.wait()
        if prc.returncode != 0:
                ...
                if os.path.exists(os.path.join(outputdir, "boot_cdt_array.c")):
                        os.remove(os.path.join(outputdir, "boot_cdt_array.c"))
                if os.path.exists(os.path.join(outputdir, "port_trace.txt")):
        ...
        if ARCH_NAME != "ipq806x":
                ...
                for entry in entries:
                        ...
                        if ARCH_NAME == "ipq5018":
                                if spruce_on_pcie_x2 != None and spruce_on_pcie_x2.text == "true":
                                        boot_settings = root_cdt.find(".//device[@id='cdb2']/props[@name='boot_settings']")
                                        # overwritting Bit 16 for spruce on pcie x2
                                        boot_settings.text = str(int(boot_settings.text) | 65536)

                                if spruce_on_pcie_x1 != None and spruce_on_pcie_x1.text == "true":
                                        boot_settings = root_cdt.find(".//device[@id='cdb2']/props[@name='boot_settings']")
        ...
        for cdt_info in os.listdir(srcDir):
                if not os.path.isdir(cdt_info):
                        if cdt_info.endswith(".xml"):
                                process_rc = process_bootldr(cdt_info)
                                if process_rc != 0:
                                        return process_rc
