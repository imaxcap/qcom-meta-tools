...
def gen_struct(format, image):
        ...
        fmt_str = "<%s" % "".join([x[1] for x in format])
        elems = struct.unpack(fmt_str, image.read(struct.calcsize(fmt_str)))
...
def elf_check(file):
        ...
        elf_hdr = gen_struct(elf_hdr, file)
        file.seek(0)

        is_elf = (elf_hdr["ident_magic"] == b"\x7fELF")
