...
def process_nand_device(pagesize, pages_per_block, total_blocks, entry, nand_type):
    ...
    partition_tool = os.path.join(cdir, 'scripts', 'partition_tool.py')
    ...
    prc = subprocess.Popen([sys.executable, mbn_gen, nand_partition,
    ...
    userpart_path = os.path.abspath(os.path.join(outputdir, nanduserbin))
    nandsyspartition_abs = os.path.abspath(nandsyspartition)
...
def process_nand(config_path, flash_type):
    ...
    if arch != None and arch.text:
        ARCH_NAME = str(arch.text)
    else:
        ARCH_NAME = os.path.basename(os.path.dirname(os.path.dirname(os.path.abspath(config_path))))
    entry = False
    QCN9000 = False
    QCN9224 = False
...
def process_emmc(config_path, flash_type):
    ...
    print('\tCreating rawprogram0.xml and patch0.xml', end=' ')
    prc = subprocess.Popen([sys.executable, ptool, '-x', emmc_partition], cwd=outputdir)
    prc.wait()
    if prc.returncode != 0:
    ...
    if ARCH_NAME == "ipq9574":
        ...
        print('\tCreating rawprogram2.xml and patch2.xml', end=' ')
        prc = subprocess.Popen([sys.executable, ptool, '-x', emmc_partition, '-p', '2'], cwd=outputdir)
        prc.wait()
        if prc.returncode != 0:
...
def process_norplusemmc(config_path, flash_type):
    ...
    print('\tCreating rawprogram1.xml and patch1.xml', end=' ')
    prc = subprocess.Popen([sys.executable, ptool, '-x', emmc_partition, '-p', '1'], cwd=outputdir)
    prc.wait()
    if prc.returncode != 0:
    ...
    print('\tRunning msp.py to update gpt_main0.bin partition', end=' ')
    prc = subprocess.Popen([
            sys.executable,
            msp,
            '-r',
    ...
    if ARCH_NAME == "ipq9574":
        ...
        print('\tCreating rawprogram3.xml and patch3.xml', end=' ')
        prc = subprocess.Popen([sys.executable, ptool, '-x', emmc_partition, '-p', '3'], cwd=outputdir)
        prc.wait()
        if prc.returncode != 0:
